...
void IceKingMonster::Render()
{
	...
	else if(GetStatus() == PST_EXT2)
	{
		...
		if(m_texs)
		{
			GameTextureManager* res = m_texs;
			//	画怪物
			int nTextIndex = 3160 + GetDirection() * 10 + m_bCurFrame;
			HTEXTURE tex = res->GetTexture(nTextIndex);
			short sOffsetX = 0;
			short sOffsetY = 0;
			int nDrawOffsetX = -1;
	...
	if(GetRenderMode() == ORM_TRANSPARENT)
	{
		if(GetStatus() != PST_EXT2)
		{
			...
			if(m_texs)
			{
				GameTextureManager* res = m_texs;
				//	画怪物
				int nTextIndex = CalMonIndex() + 500 + GetCurFrame();
				HTEXTURE tex = res->GetTexture(nTextIndex);
				short sOffsetX = 0;
				short sOffsetY = 0;
				int nDrawOffsetX = -1;
			...
			if(GetStatus() == PST_EXT1)
			{
				...
				GameTextureManager* res = m_texs;
				//	画怪物
				int nTextIndex = 3670 + 10 * GetDirection() + GetCurFrame();
				HTEXTURE tex = res->GetTexture(nTextIndex);
				short sOffsetX = 0;
				short sOffsetY = 0;
				int nDrawOffsetX = -1;
		...
		else if(GetStatus() == PST_EXT2)
		{
			...
			if(GetCurFrame() >= 0 &&
				GetCurFrame() <= 4)
			{
				int nTextIndex = 3750 + GetCurFrame();
				HTEXTURE tex = res->GetTexture(nTextIndex);
				short sOffsetX = 0;
				short sOffsetY = 0;
				int nDrawOffsetX = -1;
...
void IceKingMonster::Update(float _dt)
{
	if(GetStatus() == PST_EXT1)
	{
		if(GetStatus() == PST_EXT1 &&
...
bool IceKingMonster::IsLastFrame()
{
	if(GetStatus() == PST_EXT1)
	{
...
/*void IceKingMonster::OnPacket(const PacketHeader* _pPkt)
{
	g_xBuffer.Reset();
...
void IceKingMonster::DoPacket(const PkgObjectActionNot& _not)
{
...
void IceKingMonster::OnObjectAction(const PkgObjectActionNot& _not)
{
	if(_not.uAction == ACTION_EXT1)
	{
		m_fLastUpdateSkill = 0.0f;
