...
//////////////////////////////////////////////////////////////////////////
SuHuMonster::SuHuMonster()
{
	m_nLastAtkTarget = 0;
	m_fLastUpdateSkill = 0.0f;
}

/*void SuHuMonster::OnPacket(const PacketHeader* _pPkt)
{
	g_xBuffer.Reset();
...
void SuHuMonster::DoPacket(const PkgObjectActionNot& not)
{
...
void SuHuMonster::OnObjectAction(const PkgObjectActionNot& not)
{
	if(not.uAction == ACTION_ATTACK)
	{
		if(not.uUserId != 0)
...
void SuHuMonster::Update(float _dt)
{
	float dt = _dt;
			...
			if(m_fLastUpdateNoWeapon > /*0.08f*/fAtkInterval)
			{
				...
				if(m_nLastAtkTarget != 0)
				{
					...
					if(NULL != pHandler)
					{
						...
						if(pAni->Create(MAKELONG(pObj->GetObjx(), pObj->GetObjy()), this, NULL, 1320, 10, 50))
						{
							pAni->SetActive();
			...
			if(m_fLastUpdateSkill > /*0.2f*/0.1f)
			{
				...
				if(m_nLastAtkTarget != 0)
				{
					...
					if(NULL != pHandler)
					{
						...
						MagicElement* pAni = new MagicMonsPosAni;
						if(pAni->Create(MAKELONG(pObj->GetObjx(), pObj->GetObjy()), this, NULL, 1330, 20, 40))
						{
							pAni->SetActive();
							GameScene::sThis->InsertNewMagic(pAni);
...
void SuHuMonster::Render()
{
	if(GetStatus() == PST_SK_LEIDIANSHU)
	{
		...
		if(m_texs)
		{
			...
			//	画怪物
			int nTextIndex = 1240 + GetDirection() * 10 + m_bCurFrame;
