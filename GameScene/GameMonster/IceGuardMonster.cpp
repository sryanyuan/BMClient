...
//////////////////////////////////////////////////////////////////////////
IceGuardMonster::IceGuardMonster()
{
	m_fLastUpdateSkill = 0.0f;
}

void IceGuardMonster::Render()
{
	if(GetStatus() == PST_EXT1)
	{
		...
		if(m_texs)
		{
			...
			//	画怪物
			int nTextIndex = 850 + GetDirection() * 10 + m_bCurFrame;
			...
			{
				nTextIndex = 930 + GetDirection() * 10 + m_bCurFrame;
...
void IceGuardMonster::Update(float _dt)
{
	if(GetStatus() == PST_EXT1)
	{
		if(GetStatus() == PST_EXT1 &&
...
bool IceGuardMonster::IsLastFrame()
{
	if(GetStatus() == PST_EXT1)
	{
...
/*void IceGuardMonster::OnPacket(const PacketHeader* _pPkt)
{
	g_xBuffer.Reset();
...
void IceGuardMonster::DoPacket(const PkgObjectActionNot& _not)
{
...
void IceGuardMonster::OnObjectAction(const PkgObjectActionNot& _not)
{
	if(_not.uAction == ACTION_EXT1)
	{
		m_fLastUpdateSkill = 0.0f;
