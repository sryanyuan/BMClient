...
DreamDevourerMonster::DreamDevourerMonster()
{
	m_fLastUpdateSkill = 0;
}

void DreamDevourerMonster::Render()
{
	if(GetStatus() == PST_EXT1)
	{
		...
		if(m_texs)
		{
			...
			//	画怪物
			int nTextIndex = 3340 + GetDirection() * 10 + m_bCurFrame;
		...
		if(GetStatus() == PST_ATTACKNOWEAPON)
		{
			if(m_texs)
			{
				...
				if(m_bCurFrame >= 1)
				{
					...
					int nTextIndex = 3500 + GetDirection() * 10 + m_bCurFrame - 1;
...
void DreamDevourerMonster::Update(float _dt)
{
	if(GetStatus() == PST_EXT1)
	{
		if(GetStatus() == PST_EXT1 &&
...
bool DreamDevourerMonster::IsLastFrame()
{
	if(GetStatus() == PST_EXT1)
	{
...
/*void DreamDevourerMonster::OnPacket(const PacketHeader* _pPkt)
{
	g_xBuffer.Reset();
...
void DreamDevourerMonster::DoPacket(const PkgObjectActionNot& _not)
{
...
void DreamDevourerMonster::OnObjectAction(const PkgObjectActionNot& _not)
{
	if(_not.uAction == ACTION_EXT1)
	{
		m_fLastUpdateSkill = 0.0f;
