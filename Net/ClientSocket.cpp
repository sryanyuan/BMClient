...
CClientSocket g_xClientSocket2;
...
BOOL CClientSocket::InitSocket(HWND hWnd)
{
	WSADATA wsaData;
	int nRet;
...
BOOL CClientSocket::TimeoutServer()
{
...
BOOL CClientSocket::PreConnectToServer(HWND hWnd, CHAR* szID, INT nPort)
{
	...
	if ( m_sockClient == INVALID_SOCKET )
	{
		...
		/*if ( WSAAsyncSelect(m_sockClient, hWnd, 0x0400 + 2, FD_CONNECT|FD_READ|FD_CLOSE ) == SOCKET_ERROR)
		{
...
VOID CClientSocket::SendToServer(PacketBase *_pPacket)
{
	...
	DWORD dwSize = _pPacket->dwSize;
	_pPacket->dwSize = htonl(dwSize);
	if(SOCKET_ERROR == send(m_sockClient, (const char*)_pPacket, dwSize, 0))
	{
		wsprintf(strError, "Send() error %d\n", WSAGetLastError());
		OutputDebugString(strError);
	}
...
UINT CClientSocket::SendToServer(const char* _pszData, unsigned int _uLen)
{
	...
	if(SOCKET_ERROR == send(m_sockClient, _pszData, _uLen, 0))
	{
		wsprintf(strError, "Send() error %d\n", WSAGetLastError());
		OutputDebugString(strError);
		return 0;
